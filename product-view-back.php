...
if (isset($_GET['product_asc'])) {
	...
	$query = $pdo->prepare("SELECT `product_name`,`product_brand`,`supplier_name`, `product_amount` FROM `total_products` WHERE `store_email` =  '" . $email . "' ORDER BY `product_amount` ASC");
	$query->execute();
	while ($row = $query->fetch()) {
		echo "<tr>
						<td>" . $row['product_name'] . "</td>
